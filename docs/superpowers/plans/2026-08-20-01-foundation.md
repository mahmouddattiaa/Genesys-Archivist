...
describe('deriveNodeId', () => {
  ...
  it('normalizes Unicode so visually identical paths agree', () => {
    const composed = deriveNodeId({
      containerPath: ['caf\u00e9'],
      sourceType: 'T',
      discriminator: 'x',
    });
    const decomposed = deriveNodeId({
      containerPath: ['cafe\u0301'],
      sourceType: 'T',
      discriminator: 'x',
    });
...
> `resolveWithinRoot` guards the _lexical_ path. Symlink escape is a separate control: the bundle writer in a later plan must `realpath` the created directory and re-check containment, because a symlink planted inside the root can defeat lexical checks alone. That check has its own task in Plan 2.
...
- Produces:
  ...
  - `profileMetadataSchema` (Zod) and its inferred `interface ProfileMetadata`

> An earlier draft of this task also promised `class FileProfileRepository`, but defined neither an implementation nor a test for it. Persisting profiles to disk belongs with the OS credential store in Plan 2 — both need the native dependency that Phase 0 spike S0 selects. Corrected 2026-08-20 after an implementer declined to invent an untested class and escalated instead.
...
> Negative cases belong in the test file, never in `fixtures/`. Every fixture carrying an `$archivistSchema` pointer is validated by `npm run schema:validate`, so a deliberately-invalid fixture would fail CI permanently. Corrected 2026-08-20 after an implementer caught it in review.
...
```json
{
  ...
  "properties": {
    ...
    "captureId": {
      "type": "string",
      "pattern": "^[0-9]{4}-[0-9]{2}-[0-9]{2}T[0-9]{2}-[0-9]{2}-[0-9]{2}Z_[0-9a-z]{6}$"
    },
    ...
    "versions": {
      ...
      "properties": {
        ...
        "sourceProvider": {
          "enum": ["platform-api", "archy-cli", "architect-scripting-sdk", "manual-yaml", "fixture"]
        },
