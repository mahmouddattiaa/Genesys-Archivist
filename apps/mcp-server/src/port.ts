...
//
// `ArchivistPort` is the one seam between this server and everything that
// actually knows how to talk to Genesys, a profile store, or the capture/
// document pipelines. Two other agents are building those this wave and own
// `packages/composition/src/index.ts`, so this file exists to let the whole
// server be built and proven against a fake today and rewired to the real
// thing in wave 2 without touching a single tool. See `wire.ts` for the
// documented gap between this interface and what composition exposes now.
//
