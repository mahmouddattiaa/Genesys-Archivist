...
describe('extractVariables', () => {
  it('extracts the four flow-scoped variables', () => {
    expect(extractVariables(cfg).variables.filter((v) => v.scope === 'flow')).toHaveLength(4);
  });

  it('also extracts task-scoped variables', () => {
    expect(extractVariables(cfg).variables.some((v) => v.scope === 'task')).toBe(true);
  });

  it('gives every variable a stable id and a name', () => {
    for (const v of extractVariables(cfg).variables) {
  ...
  it('carries the secure marker without materialising a value', () => {
    ...
    expect(variables.every((v) => typeof v.secure === 'boolean')).toBe(true);
  });

  it('normalises the declared data type from __type', () => {
    expect(extractVariables(cfg).variables.some((v) => v.dataType.length > 0)).toBe(true);
  });

  it('raises no warnings against the clean 47-node fixture', () => {
    expect(extractVariables(cfg).warnings).toEqual([]);
  });

  describe('warnings (gap 2 regression)', () => {
    it('flags a non-record flow-scoped variable entry with SCHEMA_DEVIATION instead of dropping it silently', () => {
      const bad = parseFlowConfig({
        name: 'x',
        type: 'INBOUNDCALL',
        variables: ['not-an-object', { __type: 'StringVariable', id: 'v1', name: 'ok' }],
        flowSequenceItemList: [],
      });
      const { variables, warnings } = extractVariables(bad);
      expect(variables).toHaveLength(1);
      const flag = warnings.find((w) => w.code === 'SCHEMA_DEVIATION');
      expect(flag).toBeDefined();
      expect(flag?.path).toBe('/variables/0');
    });

    it('flags a non-record task-scoped variable entry with SCHEMA_DEVIATION instead of dropping it silently', () => {
      const bad = parseFlowConfig({
        name: 'x',
        type: 'INBOUNDCALL',
        variables: [],
        flowSequenceItemList: [
          { __type: 'Task', trackingId: 1, id: 'g1', name: 'T', variables: [42] },
        ],
      });
      const { variables, warnings } = extractVariables(bad);
      ...
      const flag = warnings.find((w) => w.code === 'SCHEMA_DEVIATION');
      expect(flag).toBeDefined();
      expect(flag?.path).toBe('/flowSequenceItemList/0/variables/0');
    });
  });
});
...
describe('indexVariableUsage', () => {
  it('resolves reads by variable id, not by name', () => {
    const index = indexVariableUsage(cfg, extractNodes(cfg).nodes);
